...
public:
    TreeNode *lowestCommonAncestor(TreeNode *root, TreeNode *p, TreeNode *q)
    {
        ...
        TreeNode *ans = f(root, p->val, q->val);
        return ans;
    }

    TreeNode *f(TreeNode *root, int x, int y)
    {
        // retuen when @ cur node required node
        if (root == NULL || root->val == x || root->val == y)
            return root;

        TreeNode *l = f(root->left, x, y);
        TreeNode *r = f(root->right, x, y);

        // this when req is found then cur is at some other node(possuble ansistor)
        if (l == NULL)
            return r;
