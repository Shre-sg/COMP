...
vector<int> diagonal(Node *root) {
    ...
    while (!q.empty()) {
        ...
        while (temp) {            //very important 
            ans.push_back(temp->data);  //storing our ans

            if (temp->left) {
                q.push(temp->left);  //left node is push to que
            }
...
int main() {
    ...
    // Calling the diagonal function
    vector<int> result = diagonal(root);

    // Displaying the result
    cout << "Diagonal elements of the binary tree: ";
