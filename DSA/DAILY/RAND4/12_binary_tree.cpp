...
public:
    ...
    {
        unordered_map<int, TreeNode *> map; // tree value and its node tree
        unordered_set<int> children;        // stores only children
        ...
        int root = -1; // this loop is just to find the root of the tree
        for (auto &arr : descriptions)
        {
...
int main()
{
    ...
    printTree(root); // Output should be the tree in pre-order traversal
    return 0;
}
