...
long f(int index, int buy, long *values , int n){
    ...
    if(buy){          ///when buy is det to one we are able to buy
        ...
        //profit is max( ) either you buy stock at index (-values is added) or 
        //dont buy ie ( 0 is added) called f again for nextindex;
    }
        // PROFIT = - PRICE BOUGHT  + PRICE SELLED 

    else{
        profit = max(values[index] + f(index+1, 1, values, n)  ,  0 + f(index+1, 0, values,n));
        //profit is max ( ), sell ( vales is added), next index is free to  buy,
        // or dont sell ( 0 is added) and next index is not  free to buy
    }
    return profit;
}
