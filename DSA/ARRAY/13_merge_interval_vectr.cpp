...
public:
    ...
    { // fun returns a vector inside of a vector
        ...
        { // loops thru all the vetors of interval vectoor

            // compaing the end_point of ans-vec to start_point of interval_vec if its greater we merge
            if (ans[j][1] >= intervals[i][0])
            { // comparing one ans vector to all the vector present in interval vector
